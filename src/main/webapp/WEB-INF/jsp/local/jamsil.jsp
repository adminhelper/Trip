...
<section class="page-section bg-s" id="portfolio">
    <div class="container">
        <div class="text-center">
            <h2 class="section-heading text-uppercase">잠실</h2>
            <h3 class="section-subheading text-muted">놀라움으로 가득한 서울의 녹색 쉼터</h3>
        </div>
                ...
                <div class="col-lg-3 col-sm-6 item ${data.contenttypeid}">
                    <div class="portfolio-item">
                        <a class="portfolio-link"
                           href="/local/jamsil/detail?contentid=${data.contentid}&typeid=${data.contenttypeid}">
        ...
        <ul class="pagination justify-content-center">
            <c:if test="${paging.prev}">
                <li class="page-item">
                    ...
                    <a class="page-link" style="color: black"
                       href="<c:url value='jamsil?pageNum=${paging.startPage-1}&contenttypeid=${type}}'/>" aria-label="Previous">
                        이전
                    </a>
            ...
            <c:forEach begin="${paging.startPage}" end="${paging.endPage}" var="num">
                <li class="page-item"><a class="page-link" style="color: black"
                                         href="<c:url value='jamsil?pageNum=${num}&contenttypeid=${type}'/>">${num}</a></li>
            </c:forEach>
            <c:if test="${paging.next && paging.endPage>0}">
                <li class="page-item">
                    <a class="page-link" style="color: black"
                       href="<c:url value='jamsil?pageNum=${paging.endPage+1}&contenttypeid=${type}'/>"
                       aria-label="Next">
                        다음
                    </a>
                </li>
...
<c:url var="getBoardListURL" value="/local/jamsil"></c:url>
<script>
    function onBtn(e) {
        if(e == undefined){
            location.href = "/local/jamsil";
            return ;
        }
        location.href = "/local/jamsil?contenttypeid=" + e;
        $('.item').hide();
        $('.item.' + e).show();
    }
