                ...
                <div class="col-lg-3 col-sm-6 item ${data.contenttypeid}">
                    <div class="portfolio-item">
                        <a class="portfolio-link"
                           href="/local/gwanghwamun/detail?contentid=${data.contentid}&typeid=${data.contenttypeid}">
        ...
        <ul class="pagination justify-content-center">
            <c:if test="${paging.prev}">
                <li class="page-item">
                    ...
                    <a class="page-link" style="color: black"
                       href="<c:url value='gwanghwamun?pageNum=${paging.startPage-1}&contenttypeid=${type}}'/>" aria-label="Previous">
                        이전
                    </a>
            ...
            <c:forEach begin="${paging.startPage}" end="${paging.endPage}" var="num">
                <li class="page-item"><a class="page-link" style="color: black"
                                         href="<c:url value='gwanghwamun?pageNum=${num}&contenttypeid=${type}'/>">${num}</a></li>
            </c:forEach>
            <c:if test="${paging.next && paging.endPage>0}">
                <li class="page-item">
                    <a class="page-link" style="color: black"
                       href="<c:url value='gwanghwamun?pageNum=${paging.endPage+1}&contenttypeid=${type}'/>"
                       aria-label="Next">
                        다음
                    </a>
                </li>
...
<c:url var="getBoardListURL" value="/local/gwanghwamun"></c:url>
<script>
    function onBtn(e) {
        if(e == undefined){
            location.href = "/local/gwanghwamun";
            return ;
        }
        location.href = "/local/gwanghwamun?contenttypeid=" + e;
        $('.item').hide();
        $('.item.' + e).show();
    }
